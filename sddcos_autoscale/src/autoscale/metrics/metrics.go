...
func GetCpuUsage(appId string, conf *configuration.Configuration) (float64, error) {
	...
	defer c.Close()
	q := fmt.Sprintf("SELECT MEAN(container_cpu_used) FROM container WHERE container_appname='%s' and time>now() - 62s", appId)
	res, err := queryDB(c, q)
	if err != nil {
...
func GetMemoryUsage(appId string, conf *configuration.Configuration) (float64, error) {
	// Make client
	c, err := client.NewHTTPClient(client.HTTPConfig{
		Addr: conf.MetricInfluxdb, //"http://20.26.25.120:8086",
	})
	...
	defer c.Close()
	q := fmt.Sprintf("SELECT MEAN(container_mem_used) FROM container WHERE container_appname='%s' and time>now() - 62s", appId)
	res, err := queryDB(c, q)
	if err != nil {
		//fmt.Println("Error: ", err.Error())
		return -1, err
